...
·	vLLM compatibility
·	*Beta release also supports cloud models
...
web.py — A lightweight scraper using requests + BeautifulSoup. For static pages, documentation, and fast lookups with no browser overhead.
...
Knowledge Base
A personal knowledge store Lumina can write to and query. Not the same as memory — this is for explicit reference material you want her to be able to retrieve.
Tool Self-Creation (Toolmaker)
Lumina can write new tools for herself. Give her a description of what you want, and she'll generate the tool definition, register it, and have it available in the same session. The agent's own capability surface can expand at runtime.
Meta-Cognition
