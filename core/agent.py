...
def _deliver_held_text(on_response_token, text: str, cancel_event=None,
    ...
    generation than it actually was. TOKS-STREAM-TIMING-01: this replay
    loop is never timed and never treated as a final-stream measurement by
    ANY caller -- see _finalize_completion_candidate()'s own docstring for
    why a held candidate's stream duration/tok-s is reported as
    unavailable rather than measured from either this loop or the earlier
    WORK-round request that produced the candidate (that request's whole-
    call latency isn't final-stream time either -- it can include Think
    generation, prefill, and network delay with no visible boundary
    against the actual answer text). This can only ever look like what it
    honestly is: a complete answer delivered essentially at once, through
    the same channel a real stream uses, with no throughput claimed for
    the delivery itself.

    ``on_first_chunk`` (Bino-approved TTFA expansion): an optional
    zero-arg callback fired exactly once, right before the FIRST chunk
    is actually handed to on_response_token -- this is deliberately
    "when the first held chunk is actually delivered", never "when
    delivery started" or "when the candidate was originally generated".
    See _finalize_completion_candidate()'s own use of this for why that
    distinction matters for time-to-first-answer."""
    if not text:
        return
...
def _fire_final_ttft(agent, final_ttft_s: float) -> None:
    ...
    out, just relocated to a differently-named metric. A DIFFERENT clock
    than time-to-first-answer (turn-dispatch anchored, includes
    everything before Final) and than final-stream duration (same
...
def _fire_time_to_first_answer(agent, ttfa_s: float) -> None:
    """TOKS-STREAM-TIMING-01 (Bino-approved expansion) -- time-to-first-
    answer: turn dispatch (the turn_started_at anchor threaded down from
    chat()'s wrapper) to the first nonempty Final-answer delta actually
    reaching the user -- user-perceived answer latency, deliberately
    INCLUDING Think, Commentary, tool calls, gate round trips, WORK
    rounds, and reconciliation's own earlier attempt, unlike final-stream
    duration which deliberately excludes all of that. Fires at most once
    per turn, from exactly two places: _stream_final()'s first Final
    delta (a direct or reconciliation final), or _finalize_completion_
    candidate()'s _deliver_held_text() call -- for a promoted candidate,
    "first answer" means the first REPLAYED chunk actually delivered,
    never back-dated to when the candidate was originally generated
    (that WORK round's own latency is exactly what Sol's held-candidate
    review found could not be trusted as a stream-shaped figure; it is
    still real elapsed time the user waited, which is precisely what
    TTFA counts here)."""
    on_time_to_first_answer = getattr(agent, "on_time_to_first_answer", None)
    if callable(on_time_to_first_answer):
        on_time_to_first_answer(ttfa_s)
...
class LuminaAgent:
    def __init__(self,
        ...
        Streaming callbacks:
          ...
          on_final_ttft(final_ttft_s) — TOKS-STREAM-TIMING-01 (Bino-
              approved expansion, Bino-corrected naming): Final TTFT
              (final-request time-to-first-token). The final-PRODUCING
              ...
              measurement, distinct from both on_final_stream_timing
              (that same request's Final-content generation only, a
              different boundary) and on_time_to_first_answer below
              (whole-turn, user-perceived, a different anchor entirely).
          on_time_to_first_answer(ttfa_s) — TOKS-STREAM-TIMING-01 (Bino-
              approved expansion): time-to-first-answer. Turn dispatch
              (the SAME anchor turn wall time uses) to the first
              nonempty Final-answer delta actually reaching the user --
              deliberately INCLUDES Think, Commentary, tool calls, gate
              round trips, WORK rounds, and reconciliation's own earlier
              attempt, unlike on_final_stream_timing which deliberately
              excludes all of that. Fires at most once per turn, from
              _stream_final()'s first Final delta OR (for a promoted
              held candidate) _deliver_held_text()'s first actually-
              delivered replayed chunk -- never back-dated to when that
              candidate was originally generated. Never fires for an
    ...
    def _stream_final(self, messages: list, think_step: list, cancel_event=None,
        ...
        ``turn_started_at`` (TOKS-STREAM-TIMING-01, Bino-approved
        expansion): the SAME monotonic anchor chat()'s wrapper stamps at
        turn dispatch, threaded down through _chat_impl() -- this
        method's only use for it is computing time-to-first-answer (see
        the first-Final-delta branch below). None for any caller that
        predates this threading (existing direct-call test fakes) or
        never had one to give (there is none today) -- TTFA simply never
        fires in that case, the same safe-degrade posture used
        ...
        # TOKS-STREAM-TIMING-01 -- final_stream_started_at is set ONLY at
        # the first real Final delta (the `else` branch below, never the
        # __THINK_START__/__THINK_END__/in_think branches) -- Think must
        # never start or extend this clock. Left None for the whole call
        # if no Final delta ever arrives (a pure-<think> or genuinely
        # empty stream): see the empty-content guard after the loop below,
        # which is exactly where "throughput unavailable" is decided for
        # that case.
        final_stream_started_at = None
        final_stream_ended_at = None
        # TOKS-STREAM-TIMING-01 (Final TTFT, Bino-corrected naming) -- THIS
        # request's own dispatch (right before the provider call below) to
        # the first NONEMPTY output delta of THAT SAME REQUEST -- Think or
        # Final text either counts, but a bare __THINK_START__/
        # __THINK_END__ sentinel does not (it's a protocol marker, not
        # model output) -- see the elif in_think:/else: branches below,
        # where the check actually happens, never unconditionally on
        # whatever next(stream) returns first. Scoped to THIS call only:
        # a WORK round earlier in the same turn is a separate, non-
        # streaming request this method never sees or times -- "Final" in
        # the name is load-bearing precisely to rule out that confusion.
        # A DIFFERENT anchor and a DIFFERENT question than
        # final_stream_started_at above (first FINAL delta specifically)
        # and than turn_started_at (whole-turn dispatch, used for TTFA
        # below) -- three independent clocks, never allowed to overwrite
        # one another.
        ...
        try:
            ...
            def _mark_final_ttft_if_first(text_delta):
                """TOKS-STREAM-TIMING-01 (Final TTFT) -- called from
                BOTH the elif in_think:/else: branches below (never from
                the bare __THINK_START__/__THINK_END__ sentinel
                branches), so a sentinel can never be mistaken for
                "output" and never starts this clock. Fires at most once
                (guarded by final_ttft_fired), only for a genuinely
                nonempty delta. Returns the monotonic timestamp it read
                (so the else: branch below can reuse it for
                final_stream_started_at/TTFA when THIS SAME delta is also
                the first Final delta), or None if it didn't fire this
                call -- one real clock read describing one real event,
                not two independent reads of what is actually the same
                instant."""
                nonlocal final_ttft_fired
                if final_ttft_fired or not text_delta:
                    return None
                final_ttft_fired = True
                now = time.monotonic()
            ...
            while True:
                try:
                    ...
                    break
                # SEPT-AC-R1-C01-adjacent -- cancellation observed before
                # classifying the chunk, same position this check has
                # always had here (unrelated to the Final TTFT read below,
                ...
                if chunk == "__THINK_START__":
                    ...
                    if final_stream_started_at is None:
                        ...
                        # first nonempty Final-answer delta actually
                        # reaching the user. Deliberately DOES include
                        # everything before it -- Think, Commentary, tool
                        # calls, gate round trips, WORK rounds,
                        # reconciliation's own earlier attempt -- this is
                        # user-perceived latency, not provider latency.
                        if turn_started_at is not None:
                            ttfa_s = final_stream_started_at - turn_started_at
                            if ttfa_s > 0:
                                _fire_time_to_first_answer(self, ttfa_s)
            ...
            # reached by a cancellation (_raise_cancelled() always raises
            # before falling through here) or a provider exception (the
            # except clause immediately below). None if the stream never
            # produced a single Final delta (final_stream_started_at is
            # still None) -- nothing to measure.
            if final_stream_started_at is not None:
        ...
        content = "".join(full_response).strip()
        self.ctx.add_assistant(content)
        # TOKS-STREAM-TIMING-01 -- content.strip() can be empty even when
        # final_stream_started_at fired (e.g. the only Final deltas were
        # pure whitespace) -- an empty/whitespace-only Final has no
        # meaningful throughput to report either way, matching "tool-
        # only, empty, synthetic ... finals must display throughput as
        # unavailable" regardless of which reason produced the emptiness.
        # duration_s <= 0 (a same-instant start/end reading) is guarded the
        # same way -- never divide, never fire a bogus/absurd rate.
        final_stream_duration_s = None
        final_stream_token_count = None
        if (content and final_stream_started_at is not None
                and final_stream_ended_at is not None):
            candidate_duration_s = final_stream_ended_at - final_stream_started_at
            if candidate_duration_s > 0:
                final_stream_duration_s = candidate_duration_s
                final_stream_token_count = estimate_tokens(content)
    ...
    def _finalize_completion_candidate(self, candidate: dict, *, cancel_event=None,
        ...
        answer generated" or "how responsive was the request" (both
        unanswerable here, per the reasoning above). Marked at
        _deliver_held_text()'s own first-chunk-delivered callback, not
        before this method calls it and not backdated to the candidate's
        ...
        def _mark_first_answer_chunk():
            if turn_started_at is not None:
                ttfa_s = time.monotonic() - turn_started_at
                if ttfa_s > 0:
                    _fire_time_to_first_answer(self, ttfa_s)

        if callable(on_response_token):
            _deliver_held_text(on_response_token, content, cancel_event=cancel_event,
                                on_first_chunk=_mark_first_answer_chunk)
        if _cancel_requested(cancel_event):
            raise TurnCancelled()
        self.ctx.add_assistant(content)
